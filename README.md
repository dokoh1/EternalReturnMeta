...
![매칭 시스템](ReadmeImage/Lobby.png)
![매칭 시스템](ReadmeImage/CountDown.png)
![매칭 시스템](ReadmeImage/CharacterSelect.png)
![매칭 시스템](ReadmeImage/GameScene.png)
...
네트워크 게임에서 원격 플레이어의 위치를 부드럽게 보여주는 방법은 크게 두 가지이다.

**보간(Interpolation)** — 이미 알고 있는 두 점 사이를 채워넣는다.

```
서버에서 받은 위치 A와 B가 있을 때, 그 사이를 부드럽게 이동:

  A ─────────── B
    ↑ 이 구간을 Lerp로 채움
```

확실한 데이터(서버가 보내준 위치) 사이를 채우는 것이므로 정확하지만, 항상 과거 데이터를 기반으로 하므로 **실제 위치보다 한 틱 이상 뒤처진다**.

**외삽(Extrapolation)** — 알고 있는 데이터를 바탕으로 아직 모르는 미래를 예측한다.

```
서버에서 받은 마지막 위치 B와 속도 V가 있을 때, 다음 위치를 예측:
...
미래를 추정하는 것이므로 100% 정확하지는 않지만(급정지, 방향전환 시 잠깐 어긋남), **실시간에 훨씬 가까운 위치를 보여줄 수 있다**.

### 문제

Part 7에서 다룬 위치 보간(Lerp)은 상대 캐릭터의 끊김을 줄여주지만, 근본적으로 **마지막으로 받은 서버 위치를 향해 따라가는** 보간 방식이다. 즉 항상 한 틱 이상 뒤처진 위치를 보여주게 된다.

```
보간만 사용:
  서버 실제 위치:  A ─── B ─── C ─── D
  화면에 표시:       A ── B ── C ──      (항상 한 틱 뒤)

외삽 추가:
  서버 실제 위치:  A ─── B ─── C ─── D
  화면에 표시:       B' ── C' ── D' ──   (속도 기반 예측으로 실시간에 가까움)
```

예를 들어 상대방이 오른쪽으로 일직선 이동 중이라면, 보간만 사용할 때는 내 화면에서 상대가 항상 실제보다 왼쪽에 보인다. 외삽을 추가하면 "지금 이 속도로 계속 간다면 여기쯤 있을 것"이라고 예측하여 실제 위치에 더 가깝게 표시한다.
